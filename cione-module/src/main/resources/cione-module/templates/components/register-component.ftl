<section class="cmp-login">
    ...
    <form class="form-login">
        <h2 class="title [#if isOpticaPRO]opticapro[/#if]">${i18n['cione-module.templates.components.register-component.title']}</h2>
        <div class="subtitle [#if isOpticaPRO]opticapro[/#if]"> 
            <p>${i18n['cione-module.templates.components.register-component.subtitle']}</p>
            </div>
        <div id="register-fields" class="item">
            <label class="d-flex" for="register-id">${i18n['cione-module.templates.components.register-component.username']}</label>
            <input id="register-id" type="text" required="required">
        </div>
        <span id="register-error" class="error">        	
        </span>
     
        <button id="register-btn-send" class="[#if isOpticaPRO]btn-green[#else]btn-blue[/#if]" onclick="sendRegister(); return false">${i18n['cione-module.templates.components.register-component.btn-ok']}</button>
        <button id="register-btn-back" class="[#if isOpticaPRO]btn-green[#else]btn-blue[/#if]" onclick="goToLogin(); return false" style="display:none">${i18n['cione-module.templates.components.register-component.btn-back']}</button>        
    </form>
...
<script>
	...
	function sendRegister(){			
		...
		if((registerId == "") || (registerId.length < 2)){
			$("#register-error").html("<p> ${i18n['cione-module.templates.components.login-component-pur-cione.error-login']} </p>");
			return;
		...
		$.ajax({
			url: PATH_API + "/auth/v1/register?id=" + registerId + "&lang=" + LANG,
	        type: "GET",
	        cache: false,
	        dataType: "json",	        
	        success: function(response){
	        	...
	        	var textSuccess = "${i18n['cione-module.templates.components.register-component.text-success']}";
	        	$(".subtitle").html(textSuccess);
	        },
