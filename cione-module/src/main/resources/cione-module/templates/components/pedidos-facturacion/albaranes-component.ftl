...
<section class="cmp-albaranes mobile-wrapper">
    <form id="formAlbaranes" name="formAlbaranes" method="post">
        ...
        <div class="panel-filter">
            ...
            <div class="filter order-1">
                <label>${i18n['cione-module.templates.components.albaranes-component.numalbaran']}</label>
            ...
            <div class="filter hide-mobile" style="display: none;">
                <label>${i18n['cione-module.templates.components.albaranes-component.marca']}</label>
                <input class="" id="" name="marca" type="text" value="" autocomplete="off">
            </div>
    ...
    <div class="panel-table">
        <table class="table">
            <thead>
                <tr>
                    <th>${i18n['cione-module.templates.components.albaranes-component.num-albaran']}</th>
...
<script>
    ...
    function templateAlbaran(albaran,index) {
    	var urlAlbaran = encodeURIComponent(albaran.albaran.url);
    	var enlace = PATH_API + "/private/albaranes/v1/pdf?url=" + encodeURI(albaran.albaran.url);
    	var enlacePedido = CTX_PATH + "/private/pedidos-facturacion/pedidos/consulta-pedidos.html?numPedido=" + albaran.albaran.numPedido;
    	var enlaceEnvio = CTX_PATH + "/private/pedidos-facturacion/pedidos/consulta-envios.html?numAlbaran=" + albaran.albaran.numAlbaran;
        var html = "";
        html += "<tr data-id='" + index + "'>";
        html += "<td>" + albaran.albaran.numAlbaran + "</td>";
        html += "<td class=''>" + albaran.albaran.fecha + "</td>";
        html += "<td class='hide-mobile'>" + albaran.albaran.tipo + "</td>";
        
       [#if showPrices(ctx.getUser())]
        html += "<td class=''>" + albaran.albaran.importeNeto + "</td>";
        //html += "<td class='hide-mobile'><a href='" + enlace + "'><div class='icon documento'></div></a></td>";
        
       
        
        html += '<td class="hide-mobile"><a href="#" onclick="obtenerPDF(\'' + urlAlbaran +'\')"><div class="icon documento"></div></a></td>';
       ...
       [/#if]
        html += "<td class='masinfo hide-mobile'><a class='masinforesponsive' href='javascript:void(0)' onclick='toggleAlbaran(this)'>${i18n['cione-module.templates.components.albaranes-component.more-info']}</a></td>";
        html += "</tr>";
        ...
        html += "<th>${i18n['cione-module.templates.components.albaranes-component.num-pedido']}</th>";
        html += "<th>${i18n['cione-module.templates.components.albaranes-component.descripcion2']}</th>";
        html += "<th>${i18n['cione-module.templates.components.albaranes-component.unidades']}</th>";
        html += "</tr>";
        html += "</thead>";
        html += "<tbody id='" + albaran.albaran.numAlbaran + "'>";
        
        albaran.lineas.forEach(function(linea) {
			html += "<tr>";
			html += "<td> <a href='" + enlacePedido + "'>" + albaran.albaran.numPedido + "</a></td>";
			if (linea.descripcion.length > 50 && !$(".panel-table").hasClass("mobile")) {
				html += "<td title='" + linea.descripcion + "'>" + linea.descripcion.substring(0, 50) + "...</td>";
			} else {
				html += "<td>" + linea.descripcion + "</td>";
			}
			html += "<td>" + linea.unidades + "</td>";
			//html += "<td>" + linea.marca + "</td>";
			html += "</tr>";
		});
    ...
    function templateLineaAlbaran(lineaAlbaran) {
        var html = "";
        html += "<tr>";
        html += "<td>" + lineaAlbaran.descripcion + "</td>";
